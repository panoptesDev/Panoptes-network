...
(
  ...
  set -x
  wget http://api.mainnet-beta.panoptes.org/genesis.tar.bz2
  wget --trust-server-names http://api.mainnet-beta.panoptes.org/snapshot.tar.bz2
)
...
if [[ -r $FAUCET_KEYPAIR ]]; then
  ...
  $solana_keygen new --no-passphrase -fso "$PANOPTES_CONFIG_DIR"/faucet.json
fi

if [[ -f $BOOTSTRAP_VALIDATOR_IDENTITY_KEYPAIR ]]; then
  cp -f "$BOOTSTRAP_VALIDATOR_IDENTITY_KEYPAIR" "$PANOPTES_CONFIG_DIR"/bootstrap-validator/identity.json
else
  $solana_keygen new --no-passphrase -so "$PANOPTES_CONFIG_DIR"/bootstrap-validator/identity.json
fi

$solana_keygen new --no-passphrase -so "$PANOPTES_CONFIG_DIR"/bootstrap-validator/vote-account.json
$solana_keygen new --no-passphrase -so "$PANOPTES_CONFIG_DIR"/bootstrap-validator/stake-account.json
