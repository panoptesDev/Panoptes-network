...
deployBootstrapValidator() {
  ...
  tar)
    rsync -vPrc -e "ssh ${sshOptions[*]}" "$PANOPTES_ROOT"/solana-release/bin/* "$ipAddress:$CARGO_BIN/"
    rsync -vPrc -e "ssh ${sshOptions[*]}" "$PANOPTES_ROOT"/solana-release/version.yml "$ipAddress:~/"
    ;;
  local)
...
prepareDeploy() {
  ...
  tar)
    if [[ -n $releaseChannel ]]; then
      ...
      rm -f "$PANOPTES_ROOT"/solana-release.tar.bz2
      ...
      (
        ...
        curl -L --retry 5 --retry-delay 2 --retry-connrefused \
          -o "$PANOPTES_ROOT"/solana-release.tar.bz2 "$updateDownloadUrl"
      )
      tarballFilename="$PANOPTES_ROOT"/solana-release.tar.bz2
    fi
    (
      set -x
      rm -rf "$PANOPTES_ROOT"/solana-release
      cd "$PANOPTES_ROOT"; tar jfxv "$tarballFilename"
      cat "$PANOPTES_ROOT"/solana-release/version.yml
    )
    ;;
  local)
    if $doBuild; then
  ...
  *)
  ...
  if [[ -n $deployIfNewer ]]; then
    ...
    if ! diff -q current-version.yml "$PANOPTES_ROOT"/solana-release/version.yml; then
...
deploy() {
  ...
  tar)
    networkVersion="$(
      (
        set -o pipefail
        grep "^commit: " "$PANOPTES_ROOT"/solana-release/version.yml | head -n1 | cut -d\  -f2
      ) || echo "tar-unknown"
    )"
    ;;
  local)
...
netem)
  if [[ -n $netemConfigFile ]]; then
    ...
    if [[ $netemCommand = "add" ]]; then
      for ipAddress in "${validatorIpList[@]}"; do
        "$here"/scp.sh "$netemConfigFile" solana@"$ipAddress":"$PANOPTES_HOME"
      done
    fi
