...
cat >> ~/solana/on-reboot <<EOF
  ...
  sudo RUST_LOG=info ~solana/.cargo/bin/panoptes-sys-tuner --user $(whoami) > sys-tuner.log 2>&1 &
  echo \$! > sys-tuner.pid
...
EOF
  ...
  validator|blockstreamer)
    ...
    if [[ $skipSetup != true ]]; then
      ...
      if [[ $nodeType = blockstreamer ]]; then
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/solana/config/blockstreamer-identity.json "$PANOPTES_CONFIG_DIR"/validator-identity.json
      else
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/solana/config/validator-identity-"$nodeIndex".json "$PANOPTES_CONFIG_DIR"/validator-identity.json
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/solana/config/validator-stake-"$nodeIndex".json "$PANOPTES_CONFIG_DIR"/stake-account.json
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/solana/config/validator-vote-"$nodeIndex".json "$PANOPTES_CONFIG_DIR"/vote-account.json
      fi
      net/scripts/rsync-retry.sh -vPrc \
        "$entrypointIp":~/solana/config/shred-version "$PANOPTES_CONFIG_DIR"/shred-version

      net/scripts/rsync-retry.sh -vPrc \
        "$entrypointIp":~/solana/config/bank-hash "$PANOPTES_CONFIG_DIR"/bank-hash || true

      net/scripts/rsync-retry.sh -vPrc \
        "$entrypointIp":~/solana/config/faucet.json "$PANOPTES_CONFIG_DIR"/faucet.json
    fi
    ...
    scp "$entrypointIp":~/solana/config/faucet.json "$PANOPTES_CONFIG_DIR"/
    if [[ $nodeType = blockstreamer ]]; then
...
EOF
    ...
    if [[ $skipSetup != true && $nodeType != blockstreamer && -z $maybeSkipAccountsCreation ]]; then
      ...
      panoptes --url http://"$entrypointIp":8328 catchup config/validator-identity.json
