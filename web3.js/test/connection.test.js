...
test('get program accounts', async () => {
  ...
  mockRpc.push([
    url,
    {
      ...
      params: [account0.publicKey.toBase58(), LAMPORTS_PER_PANO],
    },
    {
      error: null,
      result:
  ...
  mockRpc.push([
    ...
    {
      method: 'requestAirdrop',
      params: [account1.publicKey.toBase58(), 0.5 * LAMPORTS_PER_PANO],
    },
    {
      error: null,
      result:
...
test('get confirmed signatures for address', async () => {
  ...
  const badSlot = Number.MAX_PANO_INTEGER - 1;
  mockRpc.push([
    url,
    {
      method: 'getConfirmedSignaturesForAddress',
...
test('get confirmed block', async () => {
  ...
  mockRpc.push([
    ...
    {
      method: 'getConfirmedBlock',
      params: [Number.MAX_PANO_INTEGER],
    },
    {
      error: {
        message: `Block not available for slot ${Number.MAX_PANO_INTEGER}`,
      },
      result: null,
    },
  ]);
  await expect(
    connection.getConfirmedBlock(Number.MAX_PANO_INTEGER),
  ).rejects.toThrow(`Block not available for slot ${Number.MAX_PANO_INTEGER}`);
});
...
test('multi-instruction transaction', async () => {
  ...
  ).toBeLessThanOrEqual(LAMPORTS_PER_PANO);
...
test('https request', async () => {
  ...
  const connection = new Connection('https://devnet.panoptes.org');
  const version = await connection.getVersion();
  expect(version['solana-core']).toBeTruthy();
});
