    ...
    let (json_rpc_url, validator_list) = match cluster.as_str() {
        "mainnet-beta" => (
            value_t!(matches, "json_rpc_url", String)
                .unwrap_or_else(|_| "http://api.mainnet-beta.panoptes.org".into()),
            validator_list::mainnet_beta_validators(),
        ),
        "testnet" => (
            value_t!(matches, "json_rpc_url", String)
                .unwrap_or_else(|_| "http://testnet.panoptes.org".into()),
...
fn transact(
    ...
    info!(
        "Authorized staker balance: {} PANO",
    ...
    info!("Required fee: {} PANO", lamports_to_sol(required_fee));
...
fn main() -> Result<(), Box<dyn error::Error>> {
    ...
    if create_stake_transactions.is_empty() {
        ...
        info!(
            "{} PANO is required to create {} stake accounts",
        ...
        if source_stake_balance < source_stake_lamports_required {
            error!(
                "Source stake account has insufficient balance: {} PANO, but {} PANO is required",
                lamports_to_sol(source_stake_balance),
                lamports_to_sol(source_stake_lamports_required)
            );
            process::exit(1);
