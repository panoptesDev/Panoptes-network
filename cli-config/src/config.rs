...
impl Default for Config {
    fn default() -> Self {
        ...
        let json_rpc_url = "https://api.mainnet-beta.panoptes.org".to_string();
...
mod test {
    ...
    fn compute_websocket_url() {
        assert_eq!(
            Config::compute_websocket_url(&"http://devnet.panoptes.org"),
            "ws://devnet.panoptes.org/".to_string()
        );

        assert_eq!(
            Config::compute_websocket_url(&"https://devnet.panoptes.org"),
            "wss://devnet.panoptes.org/".to_string()
        );
