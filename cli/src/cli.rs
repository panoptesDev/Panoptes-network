...
#[cfg(test)]
use panoptes_faucet::faucet_mock::request_airdrop_transaction;
...
pub enum CliError {
    ...
    #[error("insufficient funds for fee ({0} PANO)")]
    InsufficientFundsForFee(f64),
    #[error("insufficient funds for spend ({0} PANO)")]
    InsufficientFundsForSpend(f64),
    #[error("insufficient funds for spend ({0} PANO) and fee ({1} PANO)")]
    ...
    fn test_cli_parse_command() {
        ...
        assert_eq!(
            ...
            CliCommandInfo {
                command: CliCommand::Airdrop {
                    faucet_host: None,
                    faucet_port: panoptes_faucet::faucet::FAUCET_PORT,
                    pubkey: Some(pubkey),
