...
function analyze_packet_loss {
  (
    ...
    "${REPO_ROOT}"/solana-release/bin/panoptes-log-analyzer analyze -f ./iftop-logs/ | sort -k 2 -g
  )
}
...
function wait_for_bootstrap_validator_stake_drop {
  ...
  ssh "${sshOptions[@]}" "${validatorIpList[0]}" "RUST_LOG=info \$HOME/.cargo/bin/panoptes wait-for-max-stake $max_stake --url http://127.0.0.1:8328"
}

function get_slot {
  source "${REPO_ROOT}"/net/common.sh
  loadConfigFile
  ssh "${sshOptions[@]}" "${validatorIpList[0]}" '$HOME/.cargo/bin/panoptes --url http://127.0.0.1:8328 slot'
}
...
function upload_results_to_slack() {
  ...
  GRAFANA_URL="https://metrics.panoptes.org:3000/d/monitor-${CHANNEL:-edge}/cluster-telemetry-${CHANNEL:-edge}?var-testnet=${TESTNET_TAG:-testnet-automation}&from=${TESTNET_START_UNIX_MSECS:-0}&to=${TESTNET_FINISH_UNIX_MSECS:-0}"
